...
pub fn draw(state: &MenuState, cfg: &RenderConfig, cols: u16, rows: u16, buf: &mut String) {
    ...
    // Width fits the title (50), hint (56), footer (~28), plus padding.
    ...
    let _ = write!(buf, "\x1b[{};{}H\x1b[1m{}\x1b[0m", y0 + 2, tx + 1, TITLE_LINE_1);
    let _ = write!(buf, "\x1b[{};{}H\x1b[1m{}\x1b[0m", y0 + 3, tx + 1, TITLE_LINE_2);
    ...
    for (i, item) in ITEMS.iter().enumerate() {
        ...
        let value = match i {
            ...
            4 => if cfg.mirror { "on".into() } else { "off".into() },
